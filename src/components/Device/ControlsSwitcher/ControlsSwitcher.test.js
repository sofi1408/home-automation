...
describe("<ControlsSwitcher />", () => {
  ...
  it("renders ColorControl if device type is SCALE", () => {
    const wrapper = shallow(<ControlsSwitcher deviceData={{ type: COLOR }} />);
    expect(wrapper.find(ColorControl)).toHaveLength(1);
  });
});
